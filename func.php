...
if(isset($_POST['patsub'])){
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if ($row['lockout_time'] && strtotime($row['lockout_time']) > time()) {
            ...
            // Successful login
            mysqli_query($con, "UPDATE patreg SET login_attempts = 0, lockout_time = NULL WHERE email = '$email'");
            ...
            if ($login_attempts >= 5) {
                ...
                mysqli_query($con, "UPDATE patreg SET login_attempts = $login_attempts, lockout_time = '$lockout_time' WHERE email = '$email'");
                echo("<script>alert('You have exceeded the maximum number of login attempts. Your account is locked for 5 minutes.');
                      window.location.href = 'index1.php';</script>");
            } else {
                mysqli_query($con, "UPDATE patreg SET login_attempts = $login_attempts WHERE email = '$email'");
